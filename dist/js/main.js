...
document.addEventListener('DOMContentLoaded', () => {
  document.querySelectorAll('.product-card, .sector-card, .service-card, .resource-card, .benefit-card, .pricing-card').forEach(el => {
    el.style.opacity = '0';
...
document.addEventListener('submit', function(e) {
  if (e.target && (e.target.id === 'demo-form' || e.target.id === 'contact-form')) {
    ...
    showToast(`¡${name === 'solicitud' ? 'Solicitud' : 'Mensaje'} enviado con éxito! Nos pondremos en contacto pronto.`);
    e.target.reset();
  }
});
